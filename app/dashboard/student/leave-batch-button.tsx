...
export function LeaveBatchButton({ batchId, batchName, nextBatchId }: LeaveBatchButtonProps) {
  ...
  async function handleLeave() {
    ...
    if (res?.error) {
      ...
      const redirectPath = nextBatchId
        ? `/dashboard/student?batchId=${nextBatchId}&info=${encodeURIComponent(`Successfully left the batch: ${batchName}.`)}`
        : `/dashboard/student?info=${encodeURIComponent(`Successfully left the batch: ${batchName}.`)}`;
